...
echo "Starting run_dp_full_cpp.sh with args: $@"
...
for CONFIG in "${configs[@]}"; do
  ...
  *)
  ...
  if [[ "${DRY_RUN:-0}" == "1" ]]; then
    ...
    echo "DRY RUN: Rscript \"dp_global/scripts/main_cpp.R\" \"${SAN_ARGS[@]}\""
  else
    Rscript "$PROJ_ROOT/dp_global/scripts/main_cpp.R" "${ALL_ARGS[@]}"
  fi
...
#   ./bin/run_dp_full_cpp.sh --config=fixed --SENSITIVITY_MODE=none --DP_MODE=none --DRY_RUN
