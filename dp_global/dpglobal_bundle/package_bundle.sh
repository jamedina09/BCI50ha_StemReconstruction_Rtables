...
if [ "$BUILD_BUNDLE" = true ]; then
  ...
  Rscript -e "source('${PROJECT_ROOT}/dp_global/dpglobal_bundle/dpglobal_bundle_loader.R')"
fi
...
if [ -f "${PROJECT_ROOT}/dp_global/src/transition_cost_rcpp.R" ]; then
  mkdir -p "${STAGE_DIR}/dp_global/dpglobal_bundle"
  cp -v "${PROJECT_ROOT}/dp_global/src/transition_cost_rcpp.R" "${STAGE_DIR}/dp_global/dpglobal_bundle/transition_cost_rcpp.R"
else
  echo "[package_bundle] NOTE: transition_cost_rcpp.R not found in dp_global/src; skipping"
fi
mkdir -p "${STAGE_DIR}/dp_global/dpglobal_bundle/src"
if [ -f "${PROJECT_ROOT}/dp_global/src/transition_cost_rcpp.cpp" ]; then
  cp -v "${PROJECT_ROOT}/dp_global/src/transition_cost_rcpp.cpp" "${STAGE_DIR}/dp_global/dpglobal_bundle/src/"
else
...
if command -v rsync >/dev/null 2>&1; then
  ...
  rm -rf "${STAGE_DIR}/dp_global/dpglobal_bundle/dist" || true
fi
...
Notes:
- The bundle includes a manifest at `dp_global/dpglobal_bundle/dpglobal_bundle_manifest.rds` listing suggested packages.
- Compile C++ acceleration: Rcpp::sourceCpp('dp_global/dpglobal_bundle/src/transition_cost_rcpp.cpp')
...
declare -a expected=(
  ...
  "${STAGE_DIR}/dp_global/dpglobal_bundle/dpglobal_bundle.RData"
  "${STAGE_DIR}/dp_global/dpglobal_bundle/dpglobal_bundle_manifest.rds"
  "${STAGE_DIR}/dp_global/dpglobal_bundle/transition_cost_rcpp.R"
)
missing=()
