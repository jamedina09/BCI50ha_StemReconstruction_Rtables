...
```bash
Rscript -e "Rcpp::sourceCpp('dp_global/dpglobal_bundle/src/transition_cost_rcpp.cpp')"
```

Manifest & packages

- If present, the manifest at `dp_global/dpglobal_bundle/dpglobal_bundle_manifest.rds` lists recommended packages. Install missing packages with:

```r
manifest <- readRDS('dp_global/dpglobal_bundle/dpglobal_bundle_manifest.rds')
...
Rscript -e "source('./dp_global/dpglobal_bundle/dpglobal_bundle_loader.R')"
sh dp_global/dpglobal_bundle/package_bundle.sh --build-bundle
```
...
- A copy of the `dp_global/` tree (it uses `rsync` if available) with these exclusions: `dp_global/output/`, `dp_global/dpglobal_bundle/dist/`, and `.git/` (keeps the archive free of large outputs and VCS metadata).
- `data_simulation/data/` (example inputs) from the project root.
- The bundle-local files: `dp_global/dpglobal_bundle/*` (if present) including `dpglobal_bundle.RData` and `dpglobal_bundle_manifest.rds` when available.
...
- `--build-bundle` — before packaging, runs:
    Rscript -e "source('dp_global/dpglobal_bundle/dpglobal_bundle_loader.R')"
  This generates/updates `dpglobal_bundle.RData` and `dpglobal_bundle_manifest.rds` in `dp_global/dpglobal_bundle` and ensures the tarball contains the RData.
- `--help` or `-h` — prints usage and exits.

Where the tarball is written
- `dp_global/dpglobal_bundle/dist/dpglobal_bundle_full_<timestamp>.tar.gz`
- Checksum: `dp_global/dpglobal_bundle/dist/dpglobal_bundle_full_<timestamp>.tar.gz.sha256`
...
sh dp_global/dpglobal_bundle/package_bundle.sh
...
Rscript -e "manifest <- readRDS('dp_global/dpglobal_bundle/dpglobal_bundle_manifest.rds'); pkgs <- unique(unlist(manifest$required_pkgs_by_file)); pkgs <- pkgs[!pkgs %in% installed.packages()[, 'Package']]; if (length(pkgs)) install.packages(pkgs)"

# Compile the C++ acceleration (recommended):
Rscript -e "Rcpp::sourceCpp('dp_global/dpglobal_bundle/src/transition_cost_rcpp.cpp')"

# Load the prebuilt functions (optional):
Rscript -e "load('dp_global/dpglobal_bundle/dpglobal_bundle.RData')"
```
...
load("./dp_global/dpglobal_bundle/dpglobal_bundle.RData")

# 2) Source the R wrapper (R-side wrapper for the C++ transition cost functions):
sys.source("./dp_global/dpglobal_bundle/transition_cost_rcpp.R", envir = globalenv())

# 3) Compile the C++ implementation (optional but recommended for speed):
Rcpp::sourceCpp("./dp_global/dpglobal_bundle/src/transition_cost_rcpp.cpp")
```
...
myenv <- new.env()
load("./dp_global/dpglobal_bundle/dpglobal_bundle.RData", envir = myenv)
...
# dp_global/dpglobal_bundle/dist/dpglobal_bundle_full_YYYYMMDD_HHMMSS.tar.gz
# dp_global/dpglobal_bundle/dist/dpglobal_bundle_full_YYYYMMDD_HHMMSS.tar.gz.sha256
```
...
Rscript -e "source('./dp_global/dpglobal_bundle/dpglobal_bundle_loader.R')"
# or
Rscript dp_global/dpglobal_bundle/dpglobal_bundle_loader.R
```

This produces:
- `dp_global/dpglobal_bundle/dpglobal_bundle.RData`
- `dp_global/dpglobal_bundle/dpglobal_bundle_manifest.rds`
...
# result: dp_global/dpglobal_bundle/dist/dpglobal_bundle_full_YYYYMMDD_HHMMSS.tar.gz
...
```bash
Rscript dp_global/dpglobal_bundle/verify_bundle.R
```
...
- A: This means the C++ code hasn't been compiled in the current session. Compile it with `Rcpp::sourceCpp('./dp_global/dpglobal_bundle/src/transition_cost_rcpp.cpp')` (requires `Rcpp` and a C++ toolchain).
