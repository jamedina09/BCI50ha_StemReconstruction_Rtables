...
# Usage: Rscript main.R --which_tag=2 --RUN_ALL_TAGS=TRUE --DP_MODE=map --SENSITIVITY_MODE=run --MANUAL_CORES=TRUE --MANUAL_CORES_VALUE=8
...
message("[dp_global main.R] here root: ", here::here())
message("[dp_global main.R] base_out_dir (raw): ", base_out_dir)
base_out_dir <- normalizePath(base_out_dir, winslash = "/", mustWork = FALSE)
message("[dp_global main.R] base_out_dir (normalized): ", base_out_dir)
...
print_help <- function() {
    cat("Usage: Rscript scripts/main.R [--KEY=VALUE] [--FLAG]\n")
...
for (name in names(overrides)) {
    ...
    if (!exists(name, inherits = FALSE)) {
        warning(sprintf("[dp_global main.R] Unknown override '%s' (ignored).\n", name))
        next
    }
        assign(name, overrides[[name]])
        message("[dp_global main.R] Overriding ", name, " = ", overrides[[name]])
}
...
if (exists("PROJECT_ROOT") && !is.null(PROJECT_ROOT) && nzchar(PROJECT_ROOT)) {
    message("[dp_global main.R] Using PROJECT_ROOT override: ", PROJECT_ROOT)
    base_out_dir <- normalizePath(file.path(PROJECT_ROOT, "dp_global", "output"), winslash = "/", mustWork = FALSE)
    message("[dp_global main.R] base_out_dir overridden to: ", base_out_dir)
}
...
message("[dp_global main.R] out_dir (computed): ", out_dir)
message("[dp_global main.R] getwd(): ", getwd())
...
ensure_dir <- function(path) {
    if (!dir.exists(path)) {
        ...
        message("[dp_global main.R] Created directory: ", path)
    } else {
        message("[dp_global main.R] Directory already exists: ", path)
    }
    invisible(path)
}
...
run_main <- function() {
    ...
    if (isTRUE(RUN_REALISM_REPORT) && !is.null(out)) {
        ...
        data.table::fwrite(rep0$summary, file = file.path(out_dir, paste0("tag_", which_tag, "_realism_summary.csv")))
        data.table::fwrite(rep0$by_group, file = file.path(out_dir, paste0("tag_", which_tag, "_realism_by_tag.csv")))
        data.table::fwrite(rep0$suggestions, file = file.path(out_dir, paste0("tag_", which_tag, "_realism_tuning_suggestions.csv")))
    }
    ...
    if (isTRUE(RUN_SENSITIVITY)) {
        ...
        if (isTRUE(WRITE_OUTPUTS)) {
            saveRDS(all_sweeps, file = file.path(out_dir, "simulated_all_transition_cost_sweeps.rds"))
            # data.table::fwrite(dt_all, file = file.path(out_dir, "simulated_all_transition_cost_sweeps.csv"))
            data.table::fwrite(dt_jumps, file = file.path(out_dir, "simulated_all_transition_cost_sweep_jumps.csv"))
            saveRDS(dt_jumps, file = file.path(out_dir, "simulated_all_transition_cost_jumps.rds"))
            data.table::fwrite(dt_jumps, file = file.path(out_dir, "simulated_all_transition_cost_jumps.csv"))
        }

        if (isTRUE(MAKE_ALL_SWEEPS_PDF)) {
            plot_all_sweeps_to_pdf(
                all_sweeps,
                pdf_file = file.path(out_dir, "simulated_all_transition_cost_sweeps.pdf"),
                y_scale = "delta",
...
if (sys.nframe() == 0L && isTRUE(RUN_K_SWEEP_DEMO)) {
    ...
    if (requireNamespace("ggplot2", quietly = TRUE)) {
        pp <- plot_k_sweep_join_vs_split(
            dt_sweep,
            k_max = 1000,
            out_path = here(out_dir, "k_sweep_join_vs_split_demo.pdf"),
            subtitle = basename(out_dir)
        )
    } else {
