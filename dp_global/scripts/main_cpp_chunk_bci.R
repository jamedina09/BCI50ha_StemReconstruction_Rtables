...
dp_bundle_path <- here("2_STEM_IDENTIFICATION", "dpglobal_bundle_full_20260330_004051")
...
run_main_chunked <- function() {
    ...
    # CHANGE PARAMETERS FOR PALMS
    xrun[growth_form == "palm"]
    xrun[
        growth_form %in% c("palm", "tree_fern"),
        `:=`(
            Bio_Mu_Growth = 0,
    ...
    write.csv(unique(xrun[, c("species", "growth_form", grep("^Bio_", names(xrun), value = TRUE)), with = FALSE]),
