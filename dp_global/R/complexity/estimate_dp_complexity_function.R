...
estimate_dp_complexity <- function(data,
    ...
    estimate_one_tag_dp <- function(tag_data, tag_val, species_val) {
        ...
        # Skip pruned-edge computation when any state matrix is too large for outer()
        # (e.g., 30K × 30K = 900M entries would exceed memory)
        max_constrained <- max(dp_counts$n_states_by_census, na.rm = TRUE)
        if (max_constrained <= 10000L) {
    ...
    if (isTRUE(fast)) {
        ...
        prune_candidates <- if (isTRUE(compute_all_pruned_edges)) {
            out[estimated_fallback == FALSE & n_censuses >= 2L, Tag]
        } else {
            character(0L)
        }
        ...
        if (isTRUE(use_dp_prune) && nrow(out) > 0L) {
            ...
            for (tg in dp_tags) {
                ...
                dp_est <- tryCatch(
                    estimate_one_tag_dp(tg_data, tg, sp),
                    error = function(e) {
                        cat(sprintf("[estimate_dp_complexity]   Tag %s: DP rescore skipped (%s)\n", tg, conditionMessage(e)))
                        NULL
                    }
                )
                if (!is.null(dp_est)) {
