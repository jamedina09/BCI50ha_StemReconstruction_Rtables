...
match_stems_optimal_backward <- function(tree_data, min_growth, max_growth, anchor_start) {
    ...
    # Compute mean ExactDate per CensusID and return a wide-format table
    ...
    for (c in seq.int(from = anchor_start - 1L, to = 1L, by = -1L)) {
        ...
        interval_val <- (as.numeric(pair_interval[[as.character(c + 1L)]]) - as.numeric(pair_interval[[as.character(c)]])) / 365.25
        growth_mat <- matrix(
            outer(curr_dbh, fut_dbh, FUN = function(d0, d1) (d1 - d0) / interval_val),
            nrow = n_curr,
            ncol = n_fut
        )
